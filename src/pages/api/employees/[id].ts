...
type Data = { message: string } | IEmployee;
...
const deleteEmployee = async (
  ...
  try {
    ...
    if (!employee) {
      return res
        .status(400)
        .json({ message: `No hay usuario con el id ${id}` });
    }
...
const updateEmployee = async (
  ...
  try {
    ...
    if (existeUsername && existeUsername._id.toString() !== id) {
      return res
        .status(400)
        .json({ message: "Usuario ya existe con ese username" });
    }
    ...
    const updateEmployee = await Employee.findByIdAndUpdate(id, newEmployees, {
      runValidators: true,
      new: true,
    });
